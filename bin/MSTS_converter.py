...
if __name__ == "__main__":
    ...
    parser.add_argument("-m","--mode", help="counting mode type: [single, single-expanded, fragment, fragment-middle], default=single", type=str, default="single")
    ...
    parser.add_argument("-k","--keepPosBigBedFile", help="bigBed file used to filter positions, keep position in file, warning only usable with seq < 100Mb", type=str)
...
#    parser.add_argument("--primary-only", help="Only primary mappings are considered",
 #                       action="store_true")
   ## add only mapped reads !
    ...
    if args.mode not in ["single", "single-expanded", "fragment", "fragment-middle"]:
        logging.error('Please choose the counting mode in this list: [single, single-expanded, fragment, fragment-middle]')
        sys.exit(0)
    else:
        if args.bed or args.wig or args.cov or args.size:
            i = BamConverter(args.BamFile, args.mode, args.prefix, args.bed, args.wig, args.cov, args.size, args.genome, args.window, args.keepPosBigBedFile, args.minFSize, args.maxFSize, args.minDepCov, args.maxDepCov, logLevel)
            i.convert()
        else:
