...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--threshold", help="Miminal sum of counts per condition to analyze position", type=int, default=5)
    parser.add_argument("-n", "--norm", help="Normalization method: quantile or scale, default=quantile", type=str, default="quantile")
    parser.add_argument("-y", "--nuctype", help="Nucleosome type to analyze, inclusive mode: very-well, well, fuzzy, bad", type=str, default="fuzzy")
    parser.add_argument("-b","--bed", help="export differential positions in bed file", type=str, default="")
    parser.add_argument("--merge", help="In case of several replicats with different peaks, positions could be merge to propose a differential area instead of several unique positions", action="store_true", default=False)
...
#    lPositions = ltmp 
    ...
    x = np.array(lData)
    Z = linkage(x, 'ward')
    Graphics.plotHierarchicalClustering(Z, 0.3*max(Z[:,2]), labels=labels, out="hierachical_clustering.png")
    x = np.array(lNormData)
    Z = linkage(x, 'ward')
    Graphics.plotHierarchicalClustering(Z, 0.3*max(Z[:,2]), labels=labels, out="hierachical_clustering_normalized.png")
    ...
    for i,pos in enumerate(lPositions):
        ...
        if analyze:
            ...
            mean1 = c1/nb1
            mean2 = c2/nb2
            var1 = variation([lNormData[rep][i] for rep in lConditions[0]])
            var2 = variation([lNormData[rep][i] for rep in lConditions[1]])
            FC = mean2/mean1
    ...
    print "#ref\tstart\tmean1\tCV1\tmean2\tCV2\tFC\tpval\tpaj"
    ...
    if args.bed:
        logging.info("Results exported in {}".format(args.bed))
            ...
            if args.merge:
                ...
                for pos in PosSortedlFinal[1:]:
                    if pos[0] == currentPos[0] and (pos[1]-1 >= currentPos[1]-30 and pos[1] <= currentPos[2]+30):
                        logging.info("merge: {}, {}".format(currentPos,pos))
