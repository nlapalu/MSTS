...
def export(fOut,lNucleosomes):
    ...
    with open(fOut,'a') as f:
        for nuc in lNucleosomes:
            f.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(nuc[0],nuc[1]+1,nuc[2]+1,nuc[3],nuc[4],nuc[5],nuc[7],nuc[8]))
    f.close()
...
#    lClassification = clusterClassifier(lKs)
    ...
    if args.refine:
        ...
        for Kclass in ["fuzzy", "bad"]:
            ...
            lAllNucleosomesFuzzy = [nuc for nuc in lSortedAllkNucleosomes if nuc[8] == Kclass ] 
            ...
            lKClassif = clusterClassifier2(lStats)
            # add position info
...
#    lClassification = clusterClassifier(lKs)
            ...
            Graphics.plotDistributionWithLimits([i for i in range(1,148)],lStats,lKClassif,out="{}.clusters.{}.png".format(args.prefix,Kclass), title="{} - refine {}".format(args.title,Kclass),xax=args.xax, yax=args.yax,legend=lKs)
        ...
        for i,nuc in enumerate(lSortedAllkNucleosomes):
            for j,nuc2 in enumerate(lSortedRefinedNuc[i:]):
                if (nuc[0],nuc[1]) == (nuc2[0],nuc2[1]):
                    ...
                    break
        print "BUG EXPORT"
        export("{}.nucleosomes.txt".format(args.prefix),lSortedAllkNucleosomesNew)

        if args.bed:
            for k in range(0,nbClusters):
                exportBed("{}.k{}-{}.cluster.bed".format("{}.refine".format(args.prefix),k,lKClassif[k]),[x for x in lSortedAllkNucleosomesNew if x[7] == k])
